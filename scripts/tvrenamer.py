...
def parseName(name):
    ...
    try:
        parse  = reader.parse(name)
    except:
        return {'Valid' : False}

    details = {}
    details['Valid']   = True
    details['Name']    = parse.series_name
    details['Season']  = parse.season_number
    details['Episode'] = parse.episode_numbers
    details['Date']    = parse.air_date
    details['Extra']   = parse.extra_info
    details['Group']   = parse.release_group
    details['Renamed'] = '%s [%s]' % (titlecase(parse.series_name),
                                      ' '.join(map(lambda x: str(details['Season'])+'x'+str(x), details['Episode'])))
...
if __name__ == '__main__':
    ...
    show = 'How.I.Met.Your.Mother.S08E11..HDTV.x264-LOL.[VTV]'
    pp(parseName(show))
